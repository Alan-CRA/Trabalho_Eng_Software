...
def search_movie(query, page=1):
    ...
    r = requests.get(url, headers=_headers(), params={"query": query, "page": page, "include_adult": "false", "language": "pt-BR"})
    r.raise_for_status()
    return r.json()

def movie_details(movie_id):
    url = f"{BASE}movie/{movie_id}"
    r = requests.get(url, headers=_headers(), params={"language": "pt-BR"})
    r.raise_for_status()
    return r.json()
...
def trending_movies(time_window="day", language="pt-BR"):
    ...
    r = requests.get(url, headers=_headers(), params={"language": language})
    r.raise_for_status()
    return r.json()
